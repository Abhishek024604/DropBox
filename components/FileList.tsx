...
import { Folder, Star, Trash, X, ExternalLink, PlayCircle } from "lucide-react"; // VIDEO SUPPORT: PlayCircle added
import {
  Table,
  TableHeader,
  TableColumn,
...
export default function FileList({
  ...
  const handleDownloadFile = async (file: FileType) => {
    try {
      ...
      if (file.type.startsWith("image/")) {
        const downloadUrl = `${process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}/${file.path}`;
      ...
      // VIDEO DOWNLOAD — ImageKit does NOT support video transforms like images
      // So we download directly from fileUrl
  ...
  // Function to open image/video in a new tab
  const openFileViewer = (file: FileType) => {
    ...
    // VIDEO SUPPORT — open video player
    else if (file.type.startsWith("video/")) {
      // ImageKit does not allow video transforms like images
      // So we open the fileUrl directly
  ...
  // Handle file click (image or video or folder)
  const handleItemClick = (file: FileType) => {
    if (file.isFolder) {
      navigateToFolder(file.id, file.name);
    } else if (file.type.startsWith("image/") || file.type.startsWith("video/")) {
      // VIDEO SUPPORT: open viewer for videos also
      openFileViewer(file);
    }
  };
                ...
                {filteredFiles.map((file) => (
                  <TableRow
                    ...
                    className={`hover:bg-default-100 transition-colors ${
                      ...
                      file.type.startsWith("video/") // VIDEO SUPPORT
                        ? "cursor-pointer"
                        : ""
                    }`}
                        ...
                        <div>
                          <div className="font-medium flex items-center gap-2 text-default-800">
                            ...
                            {file.type.startsWith("video/") && (
                              <Tooltip content="Click to play video">
                                <PlayCircle className="h-3 w-3 text-default-400" />
                              </Tooltip>
                            )}
                          </div>
